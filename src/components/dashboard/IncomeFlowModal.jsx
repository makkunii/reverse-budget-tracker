  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Process Income Flow">
      <form onSubmit={handleExecute} className="space-y-6 pt-2">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input 
              ...
              required 
              className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-900 rounded-2xl text-lg font-black font-mono focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" 
            />
          </div>
        </div>
        ...
        {goals.length > 0 && (
          <div className="space-y-3 border-t border-slate-100 pt-4">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
              {goals.map(goal => (
                <div key={goal.id} className="flex items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                  <span className="text-xs font-bold text-slate-700 truncate max-w-[50%]">{goal.title}</span>
                  <input 
                    type="number" 
                    placeholder="0" 
                    value={allocations[goal.id] || ''} 
                    onChange={(e) => setAllocations(prev => ({ ...prev, [goal.id]: e.target.value }))}
                    className="w-24 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono font-bold text-right focus:ring-2 focus:ring-indigo-500/20 outline-none" 
                  />
                </div>
              ))}
            </div>
        ...
        {/* Dynamic Summary */}
        <div className="bg-slate-900 text-white rounded-3xl p-5 border border-slate-800 space-y-1 shadow-lg">
